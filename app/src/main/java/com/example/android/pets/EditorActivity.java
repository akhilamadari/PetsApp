...
public class EditorActivity extends AppCompatActivity {
    ...
    private void insertPets() {
        ...
        values.put(PetsEntry.COLUMN_PET_NAME, namePet);
        values.put(PetsEntry.COLUMN_PET_BREED, breedPet);
        values.put(PetsEntry.COLUMN_PET_GENDER, genderPet);
        values.put(PetsEntry.COLUMN_PET_WEIGHT, weightPet);

        long newRowId = db.insert(PetsEntry.TABLE_NAME, null, values);
        if (newRowId == -1){
             String s = "Pet is not stored ";
             Toast.makeText(getApplicationContext(),s , Toast.LENGTH_LONG).show();
        }else{
            String s = "Pet is stored with the id " +newRowId;
            Toast.makeText(getApplicationContext(),s , Toast.LENGTH_LONG).show();
        }
